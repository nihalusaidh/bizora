...
import { signInWithGoogleNative, nextAfterAuth } from "@/lib/native-auth";
  ...
  const handleGoogleSignup = async () => {
    ...
    if (isCapacitor()) {
      ...
      try {
        ...
        const next = await nextAfterAuth();
        router.push(next);
        setTimeout(() => {
    ...
    return (
      <Card className="max-w-md">
        <CardHeader className="text-center">
          <div className="mx-auto mb-3 h-14 w-14 rounded-full bg-[#DC2626]/10 flex items-center justify-center">
            <Mail className="h-7 w-7 text-[#DC2626]" />
          </div>
